...
def release_warm_task(run_id: str | UUID, task_id: str | UUID, team_id: int, user_id: int) -> None:
    """Tear down an unactivated warm Run the user abandoned (or re-keyed on repo/branch change).

    Cancels the warm Run's Temporal workflow, marks the Run ``CANCELLED``, and soft-deletes the draft
    Task. Team + user scoped and idempotent: a missing/terminal Run or already-deleted Task is a no-op.
    """
    run = _get_visible_run(run_id, task_id, team_id)
    if run is None:
        return
    if not run.is_terminal:
        signal_workflow_completion(run.id, TaskRun.Status.CANCELLED, "warm_released")
        if not run.completed_at:
            run.completed_at = django_timezone.now()
        run.status = TaskRun.Status.CANCELLED
        run.save(update_fields=["status", "completed_at", "updated_at"])
        run.publish_stream_state_event()
    soft_delete_task(task_id, team_id, user_id)


def activate_warm_task(
    run_id: str | UUID, task_id: str | UUID, team_id: int, user_id: int, *, message: str
) -> contracts.TaskRunDTO | None:
    """Activate an idling warm Run on submit: forward the typed message to the already-running agent.

    Sets the draft Task's description to ``message`` when empty, signals the message to the idling Run
    (the existing ``user_message`` signal path — no new agent start), and drops the ``await_user_message``
    warm marker so the Run leaves the warm pool. Mirrors ``message_routing._handle_first_message``.

    Team + user scoped. Returns the run DTO, or ``None`` if the run isn't found/visible.
    """
    task = _visible_task_qs(team_id, user_id).filter(id=task_id).first()
    run = _get_visible_run(run_id, task_id, team_id)
    ...
    _activate_warm_run(run, task, team_id, message=message, artifact_ids=[])
