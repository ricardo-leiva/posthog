...
class TestReleaseWarmTask(APIBaseTest):
    def _warm_task_and_run(self, *, created_by=None, status=TaskRun.Status.QUEUED) -> tuple[Task, TaskRun]:
        ...
        run = task.create_run(mode="interactive", extra_state={"await_user_message": True})
        if status != TaskRun.Status.QUEUED:
            run.status = status
            run.save(update_fields=["status"])
        return task, run

    def test_cancels_run_and_soft_deletes_task(self):
        task, run = self._warm_task_and_run()
        with (
            patch(f"{FACADE}.signal_workflow_completion") as m_signal,
            patch.object(TaskRun, "publish_stream_state_event"),
        ):
            facade.release_warm_task(run.id, task.id, self.team.id, self.user.id)

        run.refresh_from_db()
        task.refresh_from_db()
        assert run.status == TaskRun.Status.CANCELLED.value
        assert run.completed_at is not None
        assert task.deleted is True
        m_signal.assert_called_once()

    def test_terminal_run_is_not_resignalled(self):
        task, run = self._warm_task_and_run(status=TaskRun.Status.COMPLETED)
        with patch(f"{FACADE}.signal_workflow_completion") as m_signal:
            facade.release_warm_task(run.id, task.id, self.team.id, self.user.id)

        run.refresh_from_db()
        assert run.status == TaskRun.Status.COMPLETED.value
        m_signal.assert_not_called()
        task.refresh_from_db()
        assert task.deleted is True

    def test_cross_team_release_is_noop(self):
        task, run = self._warm_task_and_run()
        other_org = Organization.objects.create(name="Other Org")
        other_team = Team.objects.create(organization=other_org, name="Other Team")

        with patch(f"{FACADE}.signal_workflow_completion") as m_signal:
            facade.release_warm_task(run.id, task.id, other_team.id, self.user.id)

        run.refresh_from_db()
        task.refresh_from_db()
        assert run.status == TaskRun.Status.QUEUED.value
        assert task.deleted is False
        m_signal.assert_not_called()

    def test_cross_user_release_does_not_delete_task(self):
        task, run = self._warm_task_and_run()
        other_user = User.objects.create(email="intruder@posthog.com", distinct_id="intruder")
        self.organization.members.add(other_user)

        with (
            patch(f"{FACADE}.signal_workflow_completion"),
            patch.object(TaskRun, "publish_stream_state_event"),
        ):
            facade.release_warm_task(run.id, task.id, self.team.id, other_user.id)

        task.refresh_from_db()
        assert task.deleted is False


class TestActivateWarmTask(APIBaseTest):
    def _warm_task_and_run(self, *, created_by=None, description="") -> tuple[Task, TaskRun]:
        task = Task.objects.create(
            team=self.team,
            title="",
    ...
    def test_signals_message_sets_description_and_drops_warm_marker(self):
        task, run = self._warm_task_and_run()
        with patch(f"{FACADE}.signal_task_run_user_message", return_value=True) as m_signal:
            dto = facade.activate_warm_task(run.id, task.id, self.team.id, self.user.id, message="fix the bug")

        assert isinstance(dto, contracts.TaskRunDTO)
        m_signal.assert_called_once()
    ...
    def test_does_not_overwrite_existing_description(self):
        task, run = self._warm_task_and_run(description="already set")
        with patch(f"{FACADE}.signal_task_run_user_message", return_value=True):
            facade.activate_warm_task(run.id, task.id, self.team.id, self.user.id, message="new text")

        task.refresh_from_db()
        assert task.description == "already set"

    def test_cross_user_activate_is_noop(self):
        task, run = self._warm_task_and_run()
        other_user = User.objects.create(email="intruder2@posthog.com", distinct_id="intruder2")
        self.organization.members.add(other_user)

        with patch(f"{FACADE}.signal_task_run_user_message") as m_signal:
            dto = facade.activate_warm_task(run.id, task.id, self.team.id, other_user.id, message="hijack")
    ...
    def test_cross_team_activate_is_noop(self):
        task, run = self._warm_task_and_run()
        other_org = Organization.objects.create(name="Other Org 2")
        other_team = Team.objects.create(organization=other_org, name="Other Team 2")

        with patch(f"{FACADE}.signal_task_run_user_message") as m_signal:
            dto = facade.activate_warm_task(run.id, task.id, other_team.id, self.user.id, message="hijack")
...
class TestRunTaskWarmActivation(APIBaseTest):
    ...
    def test_activates_idling_warm_run_without_creating_a_new_run(self):
        ...
        with patch(f"{FACADE}.signal_task_run_user_message", return_value=True) as m_signal:
            result = facade.run_task(
                ...
                self.team.id,
                self.user.id,
                validated_data={"mode": "interactive", "pending_user_message": "do the thing"},
            )
    ...
    def test_falls_back_to_description_when_no_pending_message(self):
        ...
        with patch(f"{FACADE}.signal_task_run_user_message", return_value=True) as m_signal:
            facade.run_task(task.id, self.team.id, self.user.id, validated_data={"mode": "interactive"})
