...
class WarmTaskResponseSerializer(serializers.Serializer):
    """Response for a successful warm request — the draft Task + idling warm Run to activate on submit."""

    task_id = serializers.UUIDField(
        help_text="Id of the draft Task birthed for the warm Run. Echo it back on `release_warm`/`activate_warm`.",
    )
    run_id = serializers.UUIDField(
        help_text="Id of the idling warm Run. Activate it on submit (or release it on abandon) by this id.",
    )


class ReleaseWarmTaskRequestSerializer(serializers.Serializer):
    """Request body for releasing an unactivated warm Run the user abandoned (or re-keyed)."""

    run_id = serializers.UUIDField(
        help_text="Id of the warm Run to release (as returned by `warm`).",
    )
    task_id = serializers.UUIDField(
        help_text="Id of the warm Run's draft Task (as returned by `warm`).",
    )


class ActivateWarmTaskRequestSerializer(serializers.Serializer):
    """Request body for activating an idling warm Run on submit — forwards the typed message to the agent."""

    run_id = serializers.UUIDField(
        help_text="Id of the warm Run to activate (as returned by `warm`).",
    )
    task_id = serializers.UUIDField(
        help_text="Id of the warm Run's draft Task (as returned by `warm`).",
    )
    message = serializers.CharField(
        allow_blank=False,
        help_text="The user's first message. Forwarded to the already-running agent and used as the task description when empty.",
    )
