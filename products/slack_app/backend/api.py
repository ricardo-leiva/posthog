...
def resolve_slack_user(
    ...
    *,
    ...
    try:
        ...
        # `get_slack_email_for_user` owns the cache-first-then-retry lookup
        # AND the dev-mode override (returns the test-fixture email when
        # `settings.DEBUG`); resolve_slack_user just consumes the result.
...
def get_slack_email_for_user(probe_integration: Integration, slack_user_id: str) -> str | None:
    ...
    # Dev mode: always resolve to the local test fixture user regardless of
    # what Slack returns. Lives here (rather than at the resolver call sites)
    # so callers don't each have to remember the override.
    if settings.DEBUG:
        return "test@posthog.com"
